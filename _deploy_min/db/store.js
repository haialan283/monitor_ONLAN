...
db.defaults({
    ...
    /** Dòng thứ ba: host/port tùy chọn (máy chủ khác, API, …) */
    netConfigExtra: { dns: '', port: 0 },
...
function getNetConfigExtra() {
    const v = db.get('netConfigExtra').value();
    return v && typeof v === 'object' ? v : { dns: '', port: 0 };
}

function setNetConfigExtra(nc) {
    db.set('netConfigExtra', nc).value();
    markDirty();
}
