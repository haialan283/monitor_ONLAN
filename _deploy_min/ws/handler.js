...
function attachWsHandler(wss, store, broadcast, decryptPayload, encryptPayload, onViolation, getRoleFromReq, runNetworkCheckNow) {
    ...
    function pushNetConfigToClients() {
        try {
            broadcast.broadcast({
                type: 'net_config_sync',
                netConfig: store.getNetConfig(),
                netConfigRemote: store.getNetConfigRemote(),
                netConfigExtra: typeof store.getNetConfigExtra === 'function' ? store.getNetConfigExtra() : { dns: '', port: 0 },
            });
        } catch (e) {
            console.error('net_config_sync broadcast error:', e.message);
        }
    }
            ...
            try {
                ...
                if (data.type === 'update_net_config_both') {
                    ...
                    const hasExtraFields =
                        typeof store.setNetConfigExtra === 'function' &&
                        (Object.prototype.hasOwnProperty.call(data, 'dnsExtra') ||
                            Object.prototype.hasOwnProperty.call(data, 'portExtra'));
                    if (hasExtraFields) {
                        const cur = store.getNetConfigExtra();
                        store.setNetConfigExtra({
                            dns: Object.prototype.hasOwnProperty.call(data, 'dnsExtra')
                                ? String(data.dnsExtra ?? '').trim()
                                : String(cur.dns || '').trim(),
                            port: Object.prototype.hasOwnProperty.call(data, 'portExtra')
                                ? parseInt(data.portExtra, 10) || 0
                                : Number(cur.port) || 0,
                        });
                    }
                    scheduleNetCheck();
