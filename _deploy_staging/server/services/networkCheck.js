...
function getTcpTimeoutMs() {
    const n = parseInt(process.env.NET_TCP_TIMEOUT_MS, 10);
    if (Number.isFinite(n) && n >= 200) return Math.min(n, 60000);
    return 5000;
}
 ...
 * tcp fail = không kết nối được tới cổng (từ chối, timeout, reset, …).
 * Timeout TCP mặc định 5s (WAN); ghi đè bằng NET_TCP_TIMEOUT_MS.
 * port 0 = bỏ qua TCP, chỉ coi DNS.
 */
async function probeTarget(host, port) {
    const start = Date.now();
    const timeNow = new Date().toLocaleTimeString('vi-VN');
    const tcpTimeoutMs = getTcpTimeoutMs();
        ...
        ? new Promise((res) => {
              ...
              s.once('timeout', () => {
                  s.destroy();
                      ...
                      ok: false,
                      detail: `ETIMEDOUT: no TCP handshake within ${tcpTimeoutMs}ms`,
                  });
              });
              try {
                  s.connect(port, host);
...
function startNetworkCheck(store, broadcast) {
    ...
    async function checkNetwork() {
        ...
        const extra = typeof store.getNetConfigExtra === 'function' ? store.getNetConfigExtra() : { dns: '', port: 0 };
        ...
        await checkOne(extra, netHistoryExtra, 'SYSTEM_NET_EXTRA', 'NETWORK EXTRA', 'net_status_extra', 'EXT');
    }
